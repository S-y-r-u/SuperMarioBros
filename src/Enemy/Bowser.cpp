...
void Bowser::Update(float dt) {
	...
	if (fire_cooldown_ > 0) fire_cooldown_ -= dt;
	if (jump_cooldown_ > 0) jump_cooldown_ -= dt;
	...
	if (state_ == Bowser_State::normal) {
		...
		if (fire_timer >= 0.5f) {
			...
			fire_timer = 0.0f;
			fire_cooldown_ = 5.0f; // reset cooldown bắn
		}
	}
    Fire(dt);
    Move_(dt);
...
void Bowser::Move_(float dt) {
	...
	if (position_.x < x_min_) position_.x = x_min_;
	if (position_.x > x_max_) position_.x = x_max_;
}
...
void Bowser::Fire(float dt) {
    if (fire_cooldown_ > 0) fire_cooldown_ -= dt;
	if (state_ == Bowser_State::fire) {
		// TODO: Thực hiện logic bắn lửa (tạo fireball, hiệu ứng, v.v.)
		// is_firing_ = true;
		// fire_animation_.Update(dt);
		// Sau khi bắn xong, cooldown sẽ được reset ở Update khi chuyển lại normal
	}
}
