...
void SoundManager::LoadSounds()
{
    ...
    soundEffects["invincible"] = LoadSound((path + "invincible.mp3").c_str());
    soundEffects["world_clear"] = LoadSound((path + "world_clear.wav").c_str());
...
void SoundManager::LoadMusic()
{
    ...
    musics["princess_rescued"] = LoadMusicStream((path + "princess_rescued.mp3").c_str());
    musics["finalcalc"] = LoadMusicStream((path + "finalcalc.wav").c_str());
...
void SoundManager::StopSoundEffect(const std::string &name)
{
    if (soundEffects.count(name))
    {
        StopSound(soundEffects[name]); // dừng sound hiệu ứng
    }
    else
    {
        std::cerr << "[SoundManager] SFX not found: " << name << "\n";
